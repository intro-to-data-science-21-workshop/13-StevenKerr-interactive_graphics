...
### Dynamic plot is no longer rendering...
...
### this plot refuses to move to the center >.<
...
plot_ly(viz_data, x = ~libraries, 
  ...
  layout(
    ...
    margin = list(l = 60, 
                  ...
                  pad = 5))
```
### we could consider taking a screenshot of it and including as an image
...
### maybe consider including in the script rather than the slide?  
...
.code120[
```{r cars, figures-side, eval=TRUE, message=FALSE, warning=FALSE, echo=TRUE, fig.width=5, fig.height=4}
library(ggplot2)
library(plotly)
...
### ggplot will display but not ggplotly 
### Trouble shoot(https://stackoverflow.com/questions/59144842/why-does-ggplotly-does-not-work-in-rmarkdown-the-same-way-ggplot-does)

```{r subplot, eval=TRUE, warning=FALSE, message=FALSE, echo=TRUE}
fig <- subplot(gg, p_gg)
...
.details[
```{r ggplotly3, message=FALSE, warning=FALSE, fig.width=5, fig.height=4, fig.align='center', echo=FALSE}
p_gg
```
]
</details>
...
```{r cars2, eval=TRUE, message=FALSE, warning=FALSE, fig.width=5, fig.height=4, fig.align='center', echo=FALSE}
fig <- mtcars %>% 
  plot_ly(x = ~wt) %>% 
...
- plotly can be complicated. It may get frustrating. 
<br>
</br>
- Setting up the layout requires a lot of code
<br>
</br>
