...
# Install plotly `r emo::ji("woman_technologist")`
...
fig <- plot_ly(
  ...
  columnorder = c(0, 1),
  header = list(
    values = c("ggplot","plotly"),
    align = c("center", "center"),
    line = list(width = 1, color = 'black'),
    fill = list(color = c("#434b73", "#434b73")),
    font = list(family = "Arial", size = 14, color = "white")
  ),
  cells = list(
    values = rbind(c("info on ggplot"), c("info on plotly")),
    align = c("center", "center"),
...
Like ggplot2, you start with a base and add layers
a figure has two key components: data (aka, traces) and a layout. A trace defines a mapping from data and visuals.4 Every trace has a type (e.g., histogram, pie, scatter, etc) and the trace type determines what other attributes (i.e., visual and/or interactive properties, like x, hoverinfo, name) are available to control the trace mapping.
...
There are also a family of add_*() functions (e.g., add_histogram(), add_lines(), etc) that define how to render data into geometric objects.
There are numerous other add_*() functions that calculate statistics in the browser (e.g., add_histogram2d(), add_contour(), add_boxplot(), etc)
use ~ to map a variable (i.e. column) of the dataset to an aesthetic parameter.
In addition to layout() for adding/modifying part(s) of the graph’s layout
The style() function provides a way to modify trace attribute values in a plotly object. Furthermore, you can use the plotly_build() function.
Works well with the dplyr package!


There is no right or wrong answer to exactly when you should use each type of graphic, however, there are a few rules of thumb: 

* static grapchis are quick and easy to produce. 
