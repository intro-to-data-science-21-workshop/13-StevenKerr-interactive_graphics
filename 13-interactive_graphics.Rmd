...
<details>
  <summary>
  Code Chunk Filter and Control Options
  </summary>
.details[.code70[
```{r eval=TRUE, message=FALSE, warning=FALSE, prompt=TRUE}
library(crosstalk)
...
shared_data %>% 
  plot_ly(x = ~carat, # Mapping Variables
          y = ~price,
          color = ~clarity) %>% 
  layout(
...
bscols(widths = c(2,7), # bscols call, creating custom "shiny" controls
